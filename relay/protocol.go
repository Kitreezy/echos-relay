...
const (
	...
	// KindWallRequest — просьба прислать свою стену целиком, KindWallState —
	// ответ на неё.
	//
	// Нужны потому, что росчерк, отправленный отсутствующему, пропадает: в
	// памяти релея очередей нет. Владелец стены — источник правды, и при
	// открытии стены собеседники сверяются с ним напрямую.
	KindWallRequest Kind = "wallRequest"
	KindWallState   Kind = "wallState"
)
